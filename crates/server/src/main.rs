...
async fn main() -> Result<(), WickebanError> {
    ...
    let filter_string = format!(
        "warn,tower_http={level},server={level},services={level},db={level},executors={level},deployment={level},local_deployment={level},utils={level}",
        level = log_level
    );
    ...
    auto_planner::recover_pending_approvals(&deployment.db().pool).await;
    deployment
