...
pub struct Task {
    ...
    pub interview_data: Option<String>,
...
ORDER BY t.created_at DESC"#,
        ...
        let tasks = records
            ...
            .map(|rec| TaskWithAttemptStatus {
                task: Task {
                    ...
                    interview_data: rec.interview_data,
    ...
    pub async fn find_next_eligible_child(
        ...
        sqlx::query_as!(
            Task,
            r#"SELECT t.id as "id!: Uuid", t.project_id as "project_id!: Uuid", t.title, t.description, t.status as "status!: TaskStatus", t.task_type as "task_type!: TaskType", t.parent_workspace_id as "parent_workspace_id: Uuid", t.parent_task_id as "parent_task_id: Uuid", t.sort_order as "sort_order!: i32", t.plan, t.plan_status, t.is_human as "is_human!: bool", t.routing_decision, t.interview_data, t.created_at as "created_at!: DateTime<Utc>", t.updated_at as "updated_at!: DateTime<Utc>"
               FROM tasks t
    ...
    /// Update the interview data JSON for a task (section 6.3.2).
    pub async fn update_interview_data(
        pool: &SqlitePool,
        id: Uuid,
        interview_data: &str,
    ) -> Result<(), sqlx::Error> {
        sqlx::query!(
            "UPDATE tasks SET interview_data = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1",
            id,
            interview_data
        )
        .execute(pool)
    ...
    pub async fn reset_stuck_generating_plans(pool: &SqlitePool) -> Result<Vec<Task>, sqlx::Error> {
        let tasks = sqlx::query_as!(
            ...
            r#"SELECT id as "id!: Uuid", project_id as "project_id!: Uuid", title, description,
                      ...
                      routing_decision, interview_data,
