...
async function main() {
      ...
      .then((latest) => {
        if (latest && latest !== CLI_VERSION) {
          setTimeout(() => {
            ...
            console.log(`Run: npx wickeban@latest`);
          }, 2000);
        }
      })
      .catch(() => {});
  ...
  if (isMcpMode) {
    await extractAndRun("wickeban-mcp", (bin) => {
    ...
    await extractAndRun("wickeban-review", (bin) => {
    ...
    console.log(`Starting wickeban v${CLI_VERSION}${modeLabel}...`);
    await extractAndRun("wickeban", (bin) => {
      if (platform === "win32") {
