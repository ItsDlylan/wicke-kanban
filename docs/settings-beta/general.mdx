...
When running Wickeban on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
- Wickeban runs on a remote server (VPS, cloud instance, etc.)
...
Set a prefix for auto-generated branch names. When you create a new workspace or task, Wickeban automatically creates a git branch for your changes.
...
Specify where Wickeban stores workspace data. Workspaces are created in a `.wickeban-workspaces` subdirectory within this path.
...
Configure how Wickeban alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when Wickeban runs in a background tab.
- **Push Notifications** - Receive browser notifications even when Wickeban isn't in focus. Requires browser permission when first enabled.
...
Enable or disable telemetry data collection to help improve Wickeban.
