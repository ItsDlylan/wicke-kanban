...
description: "Deploy Wickeban Cloud on any server using Docker Compose"
---

This guide covers deploying Wickeban Cloud on any Linux server using Docker Compose. This approach works with any cloud provider (AWS, DigitalOcean, Hetzner, etc.) or on-premises server.
...
Clone the Wickeban repository:

```bash
git clone https://github.com/BloopAI/wickeban.git
cd wickeban
```
...
2. You should see the Wickeban Cloud login page
